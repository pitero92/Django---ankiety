...
def create(request):
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('survey', kwargs={'survey_id': new_survey.id}))
...
def survey(request, survey_id):
    try:
        ...
        if request.method == 'GET':
            ...
            kwargs['survey_edit_form'] = forms.SurveyCreationForm(prefix="survey-creation-form", instance=user_survey)

            return render(request, 'survey.html', kwargs)
        elif request.method == 'POST':
            form = forms.SurveyCreationForm(data=request.POST, prefix="survey-creation-form", instance=user_survey)
            if form.is_valid():
                form.save()
                messages.success(request, "Dane ankiety zostały zmienione.")
                return HttpResponseRedirect(reverse('survey', kwargs={'survey_id':survey_id}))
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect(reverse('survey', kwargs={'survey_id': user_survey.id}))
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            for question in survey_questions:
                answers += "Pytanie: %s <br /> Odpowiedź: <em>%s" % (question.name, request.POST.get(str(question.id), "Brak"))
                ...
                return HttpResponseRedirect(reverse('share-survey', kwargs={'survey_id': user_survey.id}))
...
def delete_answer(request, survey_id, answer_id):
    try:
        ...
        messages.error(request, "Wybrane pytanie nie istnieje.")
    return HttpResponseRedirect(reverse('answers', kwargs={'survey_id': survey_id}))
