...
def settings(request):
    kwargs = {}
    kwargs['basic_data_change_form'] = forms.UserBasicDataChangeForm(prefix='basic_data_change_form', instance=request.user)
...
def change_basic_data(request):
    if request.method == 'POST':
        form = forms.UserBasicDataChangeForm(data=request.POST, prefix='basic_data_change_form', instance=request.user)
        if form.is_valid():
            form.save()
...
def change_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if password1 == password2 and check_password(password=old_password, encoded=request.user.password, setter=make_password(password=old_password, salt=None, hasher='default')):
